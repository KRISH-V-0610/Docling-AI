"""Chatbot + reconstruct routes — Dockyyy guidance agent and the SSE
multi-agent reconstruction pipeline. Mounted at root by backend-ai/main.py
so /api/v2/ask and /api/v2/reconstruct/stream stay unchanged."""
...
from .prompts import (
    get_formatter_instruction,
    get_reviewer_instruction,
    get_poc2_reconstruction_prompt,
)

LATEX_MODEL = os.getenv("LATEX_MODEL", "openai/gpt-oss-120b")

router = APIRouter(tags=["Chatbot + Reconstruct"])
...
# --- STREAMING ENDPOINT ---
@router.post("/api/v2/reconstruct/stream", tags=["Streaming Pipeline"], summary="Stream multi-agent manuscript formatting")
async def reconstruct_stream_endpoint(
    ...
    async def sse_generator():
        ...
        latex_converter_step = Step(
            ...
            agent=Agent(
                name="LaTeXConverterAgent", model=AgnoGroq(id=LATEX_MODEL, api_key=api_key),
