...
def _md_faq(meta, code):
    ...
    if code.api_endpoints:
        items.append(("Where is the API documentation?", "Start the server and visit `http://localhost:8000/docs`."))
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 8 — FASTAPI APP + ALL ROUTES
# ══════════════════════════════════════════════════════════════════════════════

@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("🚀 GitHub README Generator API starting")
    groq_key   = "✅ active" if os.getenv("GROQ_API_KEY")   else "⚠️  not set (template fallback)"
    github_key = "✅ active" if os.getenv("GITHUB_TOKEN")   else "⚠️  not set (60 req/hr limit)"
    logger.info("Groq API: %s", groq_key)
    logger.info("GitHub token: %s", github_key)
    if _INDEX_HTML.exists():
        logger.info("UI served at http://localhost:8000/")
    else:
        logger.warning("index.html not found at %s — UI will not be available", _INDEX_HTML)
    yield
    await session_store.purge_expired()
    logger.info("🛑 GitHub README Generator API shutting down")


app = FastAPI(
    title="GitHub README Generator API",
    description=(
        "Analyzes public GitHub repositories and generates detailed README files "
        "in LaTeX (6 templates, 18 sections) and Markdown (18 sections). "
        "AI summaries powered by Groq (llama-3.3-70b-versatile). "
        "No data is persisted — all analysis is session-scoped. "
        "UI available at GET /."
    ),
    version="2.1.0",
    lifespan=lifespan,
...
# ── UI Route — serves index.html at the root ──────────────────────────────────
...
@app.get("/rate-limit-status", tags=["Info"])
...
@app.post("/api/v1/session/create", response_model=SessionResponse, tags=["Session"])
...
@app.delete("/api/v1/session/{token}", tags=["Session"])
async def delete_session(token: str):
    deleted = await session_store.delete(token)
    return {"deleted": deleted}

@app.get("/api/v1/session/status", tags=["Session"])
...
@app.post("/api/v1/repo/analyze", response_model=RepoAnalysisResponse, tags=["Repository"])
...
@app.post("/api/v1/readme/generate", response_model=ReadmeGenerateResponse, tags=["README LaTeX"])
...
@app.post("/api/v1/readme/generate/raw", tags=["README LaTeX"])
...
@app.post("/api/v1/readme/auto", response_model=AutoGenerateResponse, tags=["README LaTeX"])
...
@app.post("/api/v1/readme/auto/raw", tags=["README LaTeX"])
...
@app.post("/api/v1/markdown/auto", response_model=MdGenerateResponse, tags=["README Markdown"])
...
@app.post("/api/v1/markdown/auto/raw", tags=["README Markdown"])
...
@app.get("/api/v1/templates/", response_model=TemplatesListResponse, tags=["Templates"])
async def list_templates():
    ...
    if __main__ == "__name__":
        import uvicorn
        uvicorn.run("main:app", host="127.0.0.1", port=4567, reload=True)